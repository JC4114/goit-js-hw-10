...
ref.countryList.addEventListener('click', onClick);
...
function onClick(e) {
  ...
  ref.countryInfo.innerHTML = createInfo(
    globalList[e.target.parentNode.dataset.index]
  );
}
...
function createList(countries) {
  const result = countries
    .map(
      ({ name, flags }, i) => `<li data-index = '${i}' class = 'item'>
